...
function open_manage_dates_dialog(frm) {
...
function _build_manage_dates_dialog(frm, constraints, suppliers_by_item) {
	...
	const d = new frappe.ui.CustomDialog({
		title: '📅  Reschedule Production Dates Chain',
		size: 'super-large',
			...
			callback(r) {
				...
				if (r.message && r.message.status === 'ok') {
					...
					po_data.forEach(entry => {
						...
						if (!row) return;
						if (entry.custom_manufacturing_type != null) row.custom_manufacturing_type = entry.custom_manufacturing_type;
						if (entry.custom_supplier           != null) row.custom_supplier           = entry.custom_supplier;
					...
					sfg_data.forEach(entry => {
						...
						if (!row) return;
						if (entry.type_of_manufacturing    != null)     row.type_of_manufacturing    = entry.type_of_manufacturing;
						if (entry.supplier                 !== undefined) row.supplier               = entry.supplier;
