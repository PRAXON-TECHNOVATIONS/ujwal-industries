...
class ReportData:
    ...
    def validate_filters(self):
        if (
            not self.filters.item_code
            and not self.filters.batches
            and not self.filters.serial_nos
        ):
            frappe.throw(
    ...
    def process_manufacture_or_repack_entry(self, row, batch_details):
        ste = frappe.db.get_value(
            "Stock Entry", row.reference_name, ["purpose", "work_order"], as_dict=True
        )
        ...
        if ste and ste.purpose in ["Manufacture", "Repack"]:
            ...
            if key not in batch_details:
                serial_no, batch_no = self.get_serial_batch_no(
                    fg_item.serial_and_batch_bundle
                )
                fg_item.update(
                    {
                        ...
                        "raw_materials": frappe._dict(
                            {(row.item_code, row.reference_name): row}
                        ),
                    }
                )
    ...
    def get_serial_batch_no(self, serial_and_batch_bundle):
        ...
        return (
            (sabb_details.serial_no, sabb_details.batch_no)
            if sabb_details
            else (None, None)
        )
